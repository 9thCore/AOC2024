...
int search_from(data &pInput, int pI, int pJ, int pLastHeight, int pTargetHeight) {
	...
	for (int i = 0; i < ORTHOGONAL_DIR_COUNT; i++) {
		result += search_from(pInput, pI + DELTA_ORTHOGONAL[i][0], pJ + DELTA_ORTHOGONAL[i][1], pLastHeight + 1, pTargetHeight);
	}
...
int search_from(data &pInput, int pI, int pJ, int pTargetHeight) {
	return search_from(pInput, pI, pJ, pInput[pI][pJ].height - 1, pTargetHeight);
}
